# -*- coding: utf-8 -*-
"""
Created on Sun Jul 11 20:07:16 2021
...
df = pd.read_csv(r'C:\Users\prane\Desktop\PS-1\json_to_csv\91342.csv').iloc[:]
...
'''
plt.plot(88,df.value.values[88],'r*',markersize=10)
...
pvalues=[]
while(i<count):
   m=M[i:min(i+10,count),0]
   
   statistic,pvalue=scipy.stats.ttest_1samp(m,mean)
...
for p in point:
    plt.plot(48+(p-1)*stride,df.value.values[48+(p-1)*stride],'r*',markersize=10)
